...
export async function POST(request: Request) {
  ...
  if (!profile?.kickChatroomId) {
    return NextResponse.json(
      { error: "Kick username が未登録です" },
      { status: 400 },
    );
  }
  ...
  await db
    ...
    .set({
      kickIsMonitoring: isStart,
      kickMonitoringStartedAt: isStart ? new Date() : null,
      updatedAt: new Date(),
    })
    .where(eq(streamerProfiles.userId, user.id));

  return NextResponse.json({ success: true, isMonitoring: isStart });
}
