...
import { allowedSoundHost, findPresetByCode, snapshotOwnMappings, toPublicPreset, describePresetDbError } from "@/lib/se/presets-db";
...
export async function PATCH(request: Request, { params }: Ctx) {
  ...
  try {
    ...
    const r = describePresetDbError(err);
    console.error("[se/presets] PATCH", r.error);
    return NextResponse.json({ error: r.error }, { status: r.status });
  }
}
...
export async function DELETE(_request: Request, { params }: Ctx) {
  ...
  try {
    ...
    const r = describePresetDbError(err);
    console.error("[se/presets] DELETE", r.error);
    return NextResponse.json({ error: r.error }, { status: r.status });
  }
}
