...
- Do not implement unofficial API access, scraping, reverse engineering, or internal protocol analysis.
  - Exception (owner decision, 2026-09-25): the whowatch comment server WebSocket (`comment_server_url` + `jwt` from `/lives/{id}`) may be connected from the streamer's own browser, for the streamer's own live or a live the streamer is currently viewing (receive-only, no DB writes for others' lives; owner decision 2026-09-25 update). The only frames sent are the Phoenix Channels subscription frames (`phx_join` / `heartbeat` / `phx_leave`; owner decision 2026-09-25 "送信も可"). No comment posting, no protocol analysis. See AGENTS.md.
